...
public class ImportServiceImpl implements ImportService {
    ...
    public void importarTodo(String rutaAulas, String rutaCursos, String rutaAlumnos) {
        ...
        // Persistir solo curso -> cascade ALL guarda también aulas y alumnos
    ...
    private Map<String, Aula> cargarAulas(String rutaAulas) {
        ...
        try {
            ...
            for (String linea : lineas) {
                ...
                if (partes.length < 3) {
                    System.out.println("Línea de aulas ignorada (faltan columnas): " + linea);
                    continue;
                }
    ...
    private Map<String, Curso> cargarCursos(String rutaCursos, Map<String, Aula> mapaAulas) {
        ...
        try {
            ...
            for (String linea : lineas) {
                ...
                // codigo;nombre;descripcion;horasTotales;activo;nivel;categoria;precio;fechaInicio;fechaFin;fechaCreacion;codigoAula
                String[] partes = linea.split(";");
                if (partes.length < 12) {
                ...
                LocalDate fInicio = LocalDate.parse(partes[8].trim());
                LocalDate fFin = LocalDate.parse(partes[9].trim());

                // fechaCreacion puede venir como "yyyy-MM-dd" o "yyyy-MM-ddTHH:mm:ss"
                String fechaCreacionStr = partes[10].trim();
                LocalDateTime fechaCreacion;
                if (fechaCreacionStr.contains("T")) {
                    fechaCreacion = LocalDateTime.parse(fechaCreacionStr);
                } else {
                    fechaCreacion = LocalDate.parse(fechaCreacionStr).atStartOfDay();
                }

                // ✅ aquí estaba el error: el código de aula es la última columna
                String codigoAula = partes[11].trim();

                Aula aula = mapaAulas.get(codigoAula);
                if (aula == null) {
                    throw new RuntimeException(
                            "No se encontró Aula con código: " + codigoAula + " para el curso " + codigo);
                }
    ...
    private void cargarAlumnos(String rutaAlumnos, Map<String, Curso> mapaCursos) {
        ...
        try {
            ...
            for (String linea : lineas) {
                ...
                // Formato: nombre;email;edad;codigoCurso
                String[] partes = linea.split(";");
                if (partes.length < 4) {
                ...
                if (curso == null) {
                    throw new RuntimeException(
                            "No se encontró Curso con código: " + codigoCurso + " para el alumno " + nombre);
                }
